...
为了适应不同速度的推理服务器，特别是本地推理服务器，系统提供了可配置的API超时设置：

- 使用`API_TIMEOUT`环境变量控制API请求的最大等待时间（秒）
- 默认为300秒（5分钟），本地推理服务器可适当增加
- 系统会自动检测本地推理服务器地址（包含localhost或127.0.0.1），并提供更详细的日志

这个设置对于以下场景特别有用：
- 使用本地大模型推理服务器，这些服务器通常处理速度较慢但成本更低
- 处理包含大量图像的请求，需要更长的推理时间
- 网络连接不稳定的环境，需要更长的请求超时时间

示例配置：
```
# 增加API超时时间到10分钟（适用于较慢的本地推理服务器）
API_TIMEOUT=600
```

## 生产环境部署

**警告**: Flask的内置开发服务器（显示的警告信息`WARNING: This is a development server. Do not use it in a production deployment.`）不适合在生产环境中使用，因为它存在性能、安全性和稳定性问题。

### 推荐的部署方式

1. **Gunicorn + Nginx** (Linux/Unix环境，推荐)
2. **uWSGI + Nginx** (复杂应用场景)
3. **Waitress** (Windows环境)
...
详细的部署指南、配置示例和最佳实践，请参阅项目中的 [deployment_guide.md](deployment_guide.md) 文件。该指南包含：
...
**重要**: 在生产环境中正确部署应用对于确保性能、安全性和可靠性至关重要。
...
本系统针对PDF处理和OCR过程进行了多项性能优化，特别适合处理大型文档：

### 多进程与并行处理

- **多进程图像处理**：使用`ProcessPoolExecutor`并行处理图像转base64编码，充分利用多核CPU
- **线程池API调用**：使用`ThreadPoolExecutor`并行处理网络IO密集型的API调用
- **配置灵活**：通过`USE_MULTIPROCESSING`环境变量可以开启或关闭多进程功能

### 内存优化

- **内存映射**：对大型图像使用`mmap`进行内存映射处理，减少内存占用
- **动态批处理**：基于系统可用内存自动计算合适的批处理大小
- **主动内存回收**：每批处理完成后主动释放内存并调用垃圾回收

### 图像处理优化

- **可调节图像质量**：通过`IMAGE_QUALITY`参数平衡OCR准确率和处理速度
- **优化图像存储**：使用`optimize=True`参数优化JPEG压缩
- **错误处理机制**：优化方法失败时自动回退到简单方法确保稳定性

### 大型PDF处理

- **分批处理**：自动将大型PDF分批处理，避免内存溢出
- **智能页数检测**：使用PyPDF2快速准确获取PDF总页数
- **进度监控**：详细的日志输出显示处理进度和资源使用情况
